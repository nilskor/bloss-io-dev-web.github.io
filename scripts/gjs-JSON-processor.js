...
if ( ARGV.length >= 2 )
{
    ...
    let field      = ARGV[0].toString()
    let field_data = ARGV[1].toString()

    let dataRow

    if ( field && field_data )
    {
        dataRow = Object.keys(pages).filter( function(row)
            {
                if ( field === 'name' )
                {
                    return pages[row].name === field_data
                }
                else if ( field ===  'sourceURL' )
                {
                    return pages[row].sourceURL === field_data
                }
                else if ( field ===  'hashTag' )
                {
                    return pages[row].hashTag === field_data
                }
                else if ( field ===  'title' )
                {
                    return pages[row].title === field_data
                }
                else if ( field ===  'injector' )
                {
                    return pages[row].injector === field_data
                }
                else if ( field ===  'description' )
                {
                    return pages[row].description === field_data
                }
                else if ( field ===  'innerHTML' )
                {
                    return pages[row].innerHTML === field_data
                }
                
            }
        )
        if ( dataRow.length > 0 ) print(dataRow[0])
    }
    //print( pages[page][property] )
}
else
{
    print(`JSON-processor needs two options passed in - field & field-data.`)
    print(`JSON data is dated: ${pages['date']}`)
}
